...
The `PRODUCTION` env var controls whether to use production database credentials or local Docker Postgres. Set `PRODUCTION=true` for deployments.

## Security & Performance

- **Rate Limiting**: Chat API uses an in-memory sliding window limiter (10 req/min)
...
- **Health Checks**: System status page monitors Database, Clerk, and Embeddings API
